...
function genInvoice(prefix = 'INV-', now = new Date()) {
  ...
  return `${prefix}${now.getFullYear()}${pad(now.getMonth() + 1)}${pad(now.getDate())}-${pad(now.getHours())}${pad(now.getMinutes())}${pad(now.getSeconds())}-${Math.random().toString(36).slice(2, 6).toUpperCase()}`;
}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ ok: true, invoiceId, ...body, wa: waResult });
  } catch (e) {
